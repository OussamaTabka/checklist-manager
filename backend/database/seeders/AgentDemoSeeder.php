...
class AgentDemoSeeder extends Seeder
{
    ...
    private function seedUserStories(array $projects, User $chef, array $templates): void
    {
        $stories = [
            ...
            [
                'project' => 'qa',
                'story_id' => 'US-QA-GENERATOR-001',
                'title' => 'Generator creates a draft checklist from reusable templates',
                'description' => 'As a chef de projet, I want the checklist generator to reuse approved checklist items before generating missing tests so our checklist library stays centralized.',
                'acceptance_criteria' => "Given approved checklist templates exist, When the chef runs the checklist generator for a matching user story, Then reusable checklist items are copied into a draft\nGiven the story has acceptance criteria not covered by templates, When generation completes, Then missing test cases are generated and added to the draft\nGiven the draft is created, When the chef reviews it, Then it remains draft until approved",
                'status' => 'in_progress',
                'priority' => 'high',
