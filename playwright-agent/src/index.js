...
function externalIdForScenario(scenario) {
    switch (scenario) {
        case 'dashboard':
            return 1;
        case 'checklists':
...
function scenarioToTestId(scenario) {
    switch (scenario) {
        case 'dashboard':
...
function buildRunSpec(scenario) {
    const now = new Date();
    const runId = `agent-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}${String(now.getSeconds()).padStart(2, '0')}`;
...
async function main() {
    ...
    try {
        ...
        const shouldRetryWithFallback = fallbackModel.length > 0 && fallbackModel !== modelUsed && isOllamaTimeoutError(err);
    ...
    if (orchestrator.exitCode !== 0) {
        throw new Error(`Orchestrator failed with code ${orchestrator.exitCode}\nSTDOUT:\n${orchestrator.stdout}\nSTDERR:\n${orchestrator.stderr}`);
    }
}
main().catch((err) => {
    console.error(err);
