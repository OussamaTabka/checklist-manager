...
type GenerationEngine = 'classification' | 'playwright-models'

type PlannerPlan = {
  mode: 'scenario' | 'generic'
  scenario?: Scenario
  reason: string
}

type GeneratedCasePlan = {
...
function splitInstructionStatements(input: GeneratorInput): string[] {
  const source = `${input.test_case_title}\n${input.test_case_text}\n${input.test_case_description}`
  const withMarkers = source.replace(
    /\b(Open page|Go to|Navigate to|Type\s+|Push\s+|Click\s+|Press\s+|Verify\s+|Test all\s+|Form Validation)\b/gi,
    '\n$1',
  )
...
function cleanInstructionValue(value: string): string {
  return value
    .trim()
    .replace(/^['"]+|['"]+$/g, '')
    .replace(/[.,;:]+$/g, '')
...
function firstQuotedSegment(value: string): string | null {
  const match = value.match(/['"]([^'"]+)['"]/)
  return match?.[1] ? cleanInstructionValue(match[1]) : null
}

function parseInstructionDrivenPlan(input: GeneratorInput, normalizedBaseUrl: string): GeneratedCasePlan | null {
  const steps: Array<Record<string, unknown>> = []
  const asserts: Array<Record<string, unknown>> = []
...
async function parseTitleHeuristicPlan(
  input: GeneratorInput,
  normalizedBaseUrl: string,
  internalChecklistTarget: boolean,
): Promise<GeneratedCasePlan | null> {
...
function buildGenericAsserts(baseUrl: string): Array<Record<string, unknown>> {
  const { hostname } = new URL(baseUrl)
...
function buildUnknownScenarioAsserts(): Array<Record<string, unknown>> {
...
function buildUnknownScenarioSteps(
  internalChecklistTarget: boolean,
...
function parseInputs(html: string): HtmlNode[] {
...
function parseButtons(html: string): HtmlNode[] {
...
function parseLabelMap(html: string): Map<string, string> {
...
function selectorFromNode(tag: 'input' | 'button', node: HtmlNode, fallbackLabel: string): Record<string, unknown> {
...
function scoreFromHints(value: string, hints: string[]): number {
  const normalized = value.toLowerCase()
  return hints.reduce((score, hint) => score + (normalized.includes(hint) ? 4 : 0), 0)
}

function scoreInputCandidate(node: HtmlNode, labelText: string, hints: string[], disallowPassword: boolean): number {
  const attrs = node.attrs
...
function pickInputByHints(inputs: HtmlNode[], labelsById: Map<string, string>, hints: string[], disallowPassword: boolean): HtmlNode | null {
...
function pickSubmitButton(buttons: HtmlNode[]): HtmlNode | null {
  let best: HtmlNode | null = null
  let bestScore = -1
...
function extractCredential(text: string, key: 'username' | 'password'): string | null {
  const patterns =
    key === 'username'
      ? [
...
function extractSuccessUrlFragment(testCaseText: string): string {
...
async function detectLoginSelectorsWithCodegen(targetUrl: string): Promise<{
  ...
  try {
    ...
    const usernameInput =
      pickInputByHints(inputs, labelsById, ['username', 'user', 'email', 'login'], true) ??
      inputs.find((input) => (input.attrs.type ?? 'text').toLowerCase() !== 'password') ??
      null

    const passwordInput =
      pickInputByHints(inputs, labelsById, ['password', 'pass', 'pwd'], false) ??
...
async function buildExternalLoginFlowFromCodegen(
  input: GeneratorInput,
  normalizedBaseUrl: string,
): Promise<GeneratedCasePlan> {
  ...
  try {
    ...
    process.stderr.write(
      `[agent][codegen] selector detection failed (${error instanceof Error ? error.message : String(error)}); using resilient login fallbacks\n`,
    )
...
function plannerModel(input: GeneratorInput, internalChecklistTarget: boolean): PlannerPlan {
...
async function generatorModel(
  ...
  if (instructionPlan) {
    process.stderr.write(
      `[agent][instructions] parsed explicit steps/asserts from test case text (steps=${instructionPlan.steps.length}, asserts=${instructionPlan.asserts.length})\n`,
    )
  ...
  if (titlePlan) {
    process.stderr.write(
      `[agent][title-heuristic] inferred script from title (steps=${titlePlan.steps.length}, asserts=${titlePlan.asserts.length})\n`,
    )
    return titlePlan
  }
...
function scenarioToAssert(scenario: Scenario): Array<Record<string, unknown>> {
  switch (scenario) {
    case 'dashboard':
...
function recoverUnknownWithHeuristic(
  ...
  process.stderr.write(
    `[agent] Unknown classification recovered by heuristic for test case ${input.external_id}: ${inferred}\n`,
  )
...
function applyConfidenceThreshold(outcome: ClassificationOutcome, input: GeneratorInput): ClassificationOutcome {
  if (
    ...
    if (inferred !== 'unknown') {
      process.stderr.write(
        `[agent] Low-confidence classification (${outcome.confidence.toFixed(3)}) for test case ${input.external_id}; using heuristic scenario '${inferred}' instead of unknown.\n`,
      )
    ...
    process.stderr.write(
      `[agent] Low-confidence classification (${outcome.confidence.toFixed(3)}) for test case ${input.external_id}; threshold=${config.ollamaMinConfidence.toFixed(3)}. Routing to unknown.\n`,
    )
...
function inferScenarioFromText(input: GeneratorInput): Scenario {
  ...
  const hasLoginCue =
    /\blog\s*in\b|\blogin\b|\bauth\w*\b|\bsign\s*in\b|\bsession\b|\breset\s*password\b|\bbearer\s*token\b/.test(text)

  const hasUiCue =
    /\bpage\b|\btable\b|\bbutton\b|\bform\b|\bscreen\b|\bview\b|\broute\b/.test(text)

  const hasApiCue =
    /\bjson\b|\bschema\b|\brequest\b|\bresponse\b|\bstatus\s*codes?\b|\berror\s*codes?\b|\bheaders?\b|\bcors\b|\brate\s*limit\b|\bthrottl\w*\b|\bendpoint\b|\bpayload\b|\bbearer\s*token\b|\bhttp\b/.test(text)
  ...
  const matchedScenarios = new Set<Scenario>()
  ...
  if (matchedScenarios.size === 1) {
    return Array.from(matchedScenarios)[0] as Scenario
  }
...
async function classifyWithFallback(input: GeneratorInput): Promise<ClassificationOutcome> {
  ...
  try {
    ...
    if (!shouldRetry) {
      const inferred = inferScenarioFromText(input)
      process.stderr.write(
        `[agent] Ollama classification unavailable (${error instanceof Error ? error.message : String(error)}). Falling back to heuristic scenario: ${inferred}\n`,
      )
      return {
        scenario: inferred,
        confidence: 0,
    ...
    process.stderr.write(
      `[agent] Primary model ${modelUsed} timed out; retrying with ${fallbackModel}\n`,
    )
    ...
    try {
      ...
      const inferred = inferScenarioFromText(input)
      process.stderr.write(
        `[agent] Ollama fallback model ${fallbackModel} failed (${retryError instanceof Error ? retryError.message : String(retryError)}). Using heuristic scenario: ${inferred}\n`,
      )
      return {
        scenario: inferred,
        confidence: 0,
...
function buildRunSpec(input: GeneratorInput, scenario: Scenario) {
  ...
  if (!internalChecklistTarget && scenario !== 'unknown') {
    process.stderr.write(
      `[agent] External target detected (${normalizedBaseUrl}); running scenario '${scenario}' assertions against configured app host.\n`,
    )
  }

  if (useExternalGenericUnknown) {
    process.stderr.write(
      `[agent] External target detected (${normalizedBaseUrl}) with unknown scenario; using generic website assertions.\n`,
    )
  }
  ...
  let asserts: Array<Record<string, unknown>>

  if (useExternalGenericUnknown) {
    asserts = buildGenericAsserts(normalizedBaseUrl)
  } else if (effectiveScenario === 'unknown') {
    process.stderr.write(
      `[agent] Unknown scenario for test case ${input.external_id}; applying fail-closed guard assertion (${UNKNOWN_SCENARIO_GUARD_TOKEN}).\n`,
    )
...
async function buildRunSpecWithPlaywrightModels(input: GeneratorInput) {
  const normalizedBaseUrl = normalizeBaseUrl(input.base_url)
  const internalChecklistTarget = isInternalChecklistTarget(normalizedBaseUrl)

  const plan = plannerModel(input, internalChecklistTarget)
  process.stderr.write(
    `[agent][planner] mode=${plan.mode}; scenario=${plan.scenario ?? 'n/a'}; reason=${plan.reason}\n`,
  )

  const generated = await generatorModel(input, normalizedBaseUrl, internalChecklistTarget, plan)
  process.stderr.write(
    `[agent][generator] steps=${generated.steps.length}; asserts=${generated.asserts.length}; use_auth=${generated.useAuth}\n`,
  )

  const healed = healerModel(normalizedBaseUrl, generated)
  process.stderr.write(
    `[agent][healer] steps=${healed.steps.length}; asserts=${healed.asserts.length}\n`,
  )
...
async function main(): Promise<void> {
  ...
  const generationEngine =
    config.generationEngine === 'classification' ? 'classification' : 'playwright-models'

  process.stderr.write(`[agent] Generation engine: ${generationEngine}\n`)

  let runSpec: ReturnType<typeof buildRunSpec>
