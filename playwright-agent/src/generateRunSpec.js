...
function mentionsAny(text, patterns) {
    return patterns.some((pattern) => pattern.test(text));
}
function quotedPhrasesFromText(text) {
    return Array.from(text.matchAll(/["'“”‘’]([^"'“”‘’]{3,80})["'“”‘’]/g))
        .map((match) => match[1]?.trim() ?? '')
        .filter((value) => value.length >= 3);
}
function extractExpectedFeedbackText(input) {
    const text = normalizeText(input);
...
function actionSelectorForInput(input, coverageType) {
    const text = normalizeText(input);
    if (coverageType === 'auth_login') {
...
function buildStepsAndAsserts(input, coverageType, scenario, internalTarget) {
    ...
    const asserts = [];
    const alertSelector = { by: 'css', value: '[role="alert"], .error, .alert, .invalid-feedback, .field-error, .toast, .notification, .success' };
    const successSelector = { by: 'css', value: '[role="alert"], .success, .toast, .notification, [data-testid*="success"], [data-testid*="toast"]' };
    const listSelector = { by: 'css', value: 'table, [role="table"], [role="grid"], ul, ol, [data-testid*="list"], [data-testid*="table"]' };
    switch (coverageType) {
        case 'auth_login':
