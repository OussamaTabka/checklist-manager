...
function parseIntStrict(value, fieldName, defaultValue) {
    if (value === undefined || value.trim() === '') {
...
function loadConfig() {
    ...
    const includesProtectedSmokePages = smokePages.some((page) => ['dashboard', 'checklists', 'projects', 'users'].includes(page));
    const requestedEnableLoginFlow = parseBool(process.env.ENABLE_LOGIN_FLOW, true);
    const loginFlowForcedForProtectedSmoke = authMode === 'api' && includesProtectedSmokePages && !requestedEnableLoginFlow;
    const effectiveEnableLoginFlow = authMode === 'api' && includesProtectedSmokePages ? true : requestedEnableLoginFlow;
    ...
    return {
        ...
        smokePages,
        runsRoot: node_path_1.default.resolve(process.cwd(), process.env.RUNS_ROOT ?? process.env.RUNS_DIR ?? './runs'),
        ...
        smokeUiLoginExternalId: parseIntStrict(process.env.SMOKE_UI_LOGIN_EXTERNAL_ID, 'SMOKE_UI_LOGIN_EXTERNAL_ID', 7102),
        ...
        dockerUseHostGateway: parseBool(process.env.DOCKER_USE_HOST_GATEWAY, process.platform === 'linux'),
