      ...
      <div class="card mb-4">
        <div class="card-header">
          <div class="d-flex justify-content-between align-items-center">
            ...
            @if(Route::has('checklists.add-test-case'))
                ...
                <tbody>
                  @foreach($testCases as $testCase)
                    <tr>
                      ...
                      <td>
                        @if(Route::has('checklists.edit-test-case'))
                          <a href="{{ route('checklists.edit-test-case', [$checklist->id, $testCase->id]) }}" 
                             class="btn btn-sm btn-outline-primary">
                            Edit
                          </a>
                        @endif
                        @if(Route::has('checklists.delete-test-case'))
        ...
        <div class="card-body">
          ...
          <div class="mb-3">
            ...
            <p class="h4 text-success">
              {{ collect($testCases)->where('status', 'passed')->count() }}
            </p>
          </div>
          <div class="mb-3">
            <small class="text-muted">Failed</small>
            <p class="h4 text-danger">
              {{ collect($testCases)->where('status', 'failed')->count() }}
            </p>
          </div>
          <div>
            <small class="text-muted">Pending</small>
            <p class="h4 text-secondary">
              {{ collect($testCases)->where('status', 'pending')->count() }}
            </p>
          </div>
        </div>
      </div>
